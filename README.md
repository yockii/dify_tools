...
4. 系统中创建应用，并根据系统接口开发应用接入，此时就可以让这个系统拥有dify的能力，并且扩展了可以查询知识库和应用对应数据了。可以把`docs/common_flow.yml`导入到工作室中（当然，模型调用和动态知识库需要重新选择一下）
...
- app_secret主要为了识别应用，本系统直接调用会为空，第三方系统接入则应该有值
- custom_id为了区分用户，一般以用户ID来传
