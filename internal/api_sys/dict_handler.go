...
func (h *DictHandler) Create(c *fiber.Ctx) error {
	...
	go h.logService.CreateOperationLog(c.Context(), user.ID, model.LogActionCreateDict, c.IP(), c.Get("User-Agent"))
...
func (h *DictHandler) Update(c *fiber.Ctx) error {
	...
	// 记录操作日志
	user := c.Locals("user").(*model.User)
	go h.logService.CreateOperationLog(c.Context(), user.ID, model.LogActionUpdateDict, c.IP(), c.Get("User-Agent"))
...
func (h *DictHandler) Delete(c *fiber.Ctx) error {
	...
	// 记录操作日志
	user := c.Locals("user").(*model.User)
	go h.logService.CreateOperationLog(c.Context(), user.ID, model.LogActionDeleteDict, c.IP(), c.Get("User-Agent"))
